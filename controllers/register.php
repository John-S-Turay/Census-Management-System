    ...
    if (!empty($firstname) && !empty($lastname) && !empty($email) && !empty($mobilenumber) && !empty($password)) {
        ...
        if ($rowCount > 0) {
            ...
            if (!preg_match("/^[0-9]{9}$/", $_mobile_number)) {
                $_mobileErr = '<div class="alert alert-danger">Only 9-digit mobile numbers allowed.</div>';
            }
            if (!preg_match("/^(?=.*\d)(?=.*[@#\-_$%^&+=§!\?])(?=.*[a-z])(?=.*[A-Z])[0-9A-Za-z@#\-_$%^&+=§!\?]{8,20}$/", $_password)) {
                $_passwordErr = '<div class="alert alert-danger">Password should be between 8 to 20 characters long, contain at least one special character, lowercase, uppercase and a digit.</div>';
            }

            // Store the data in db if all the preg_match conditions are met
            if (
                preg_match("/^[a-zA-Z ]*$/", $_first_name) &&
                preg_match("/^[a-zA-Z ]*$/", $_last_name) &&
                filter_var($_email, FILTER_VALIDATE_EMAIL) &&
                preg_match("/^[0-9]{9}$/", $_mobile_number) &&
                ...
                // Query
                $sql = "INSERT INTO users (firstname, lastname, email, mobilenumber, password, token, is_active, date_time) VALUES ('$firstname', '$lastname', '$email', '$mobilenumber', '$password_hash', '$token', '0', NOW())";
                ...
                if ($sqlQuery) {
                    $msg = 'Click on the activation link to verify your email. <br><br>
                      <a href="http://localhost/form/user_verification.php?token=' . $token . '"> Click here to verify email</a>';
                    ...
                    $message = (new Swift_Message('Please Verify Email Address!'))
                        ->setFrom(['your-email@gmail.com' => $firstname . ' ' . $lastname])
                        ->setTo($email)
                    ...
                    if (!$result) {
                        $email_verify_err = '<div class="alert alert-danger">Verification email could not be sent!</div>';
                    } else {
        ...
        if (empty($mobilenumber)) {
            $mobileEmptyErr = '<div class="alert alert-danger">Mobile number cannot be blank.</div>';
        }
        if (empty($password)) {
